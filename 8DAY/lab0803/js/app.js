...
/**
 * async await 이해하기
 * - async함수는 es8(ecmascript 2017)에서 도입되었다.
 * - async함수는 함수 안의 await 구문과 함께 비동기 작업을 제어한다.
 * - await 키워드는 반드시 async 함수 안에서만 유효하다.
 * 
 * async 함수의 동작방식
 * - 처음 async 함수가 호출되어 awit 키워드가 없는 비동기 작업(Promise)이 실행되면,
 *   해당 비동기 함수는 이벤트 루프를 통해 비동기로 작업을 처리한다. 
 * - 그동안 async 함수는 이러는 비동기 작업이 완료될 때까지 일시중지 상태로 비동기 작업(Promise)의
 *   해결(resolve)을 기다린다.
 * - 이 작업이 완료되면 async 함수가 다시 실행되고 함수 결과를 반환한다.
 * 
 * async 함수를 선언하는 방법
 * - async 함수 선언문(async function)과 표현식(async function exxpression)이 있다.
